...
int main() {
  ...
  Set assigner(9), assignee(3);
  test_log("PART 1: memory exchanging test", ' ');
  assigner.insert("1");
  assignee = assigner;  // assignment operator called here
  // fill up assignee's memory
  assert(! assignee.insert("1"));
  // will fail above assertion if the content from assigner is not copied.
  assert(assignee.insert("2"));
  assert(assignee.insert("3"));
  assert(assignee.insert("4"));
  // will fail above assertion if the capacity is not expanded.
  // (i.e. the memory array is not recreated with a larger capacity)
  assert(assignee.insert("5"));
  assert(assignee.insert("6"));
  assert(assignee.insert("7"));
  assert(assignee.insert("8"));
  assert(assignee.insert("9"));
  // since the assignee's memory size has been changed to 3,
  // it should be full by this point.
  assert(! assignee.insert("0"));  // should be full, can't do it.
  test_log("passed!", ' ');
