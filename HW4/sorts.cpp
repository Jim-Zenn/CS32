...
{
  // Using the insertion sort algorithm (pp. 311-313), sort s according to the //// ordering relationship passed in as the parameter comp.
...
int main()
{
  ...
  // destruction time.
  {
    ...
    // Create a vector of Store pointers, and set each pointer to point to the
    // corresponding Store in auxStores.
    vector<Store*> ptrStores;
    for (auto &s : stores)
        ptrStores.push_back(&s);

    // Sort the vector of pointers using the STL sort algorithm with
    // compareStorePtr as the ordering relationship.
    sort(ptrStores.begin(), ptrStores.end());

    // Using the now-sorted vector of pointers, replace each Store in stores
    // with the Stores from auxStores in the correct order.
    for (size_t i = 0; i < ptrStores.size(); i ++)
