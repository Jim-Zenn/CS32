...
private:
    ...
    class GenomeRef {
    public:
        GenomeRef(string const &name, int const &keyPosition)
        : m_name(name), m_position(keyPosition) { }
        string name() const { return m_name; }
        int position() const { return m_position; }
    private:
        string m_name;  // genome name
    ...
    Trie<GenomeRef> m_index;
    map<string, Genome> m_library;
};
...
{
    m_library.emplace(genome.name(), genome);
    ...
    for (int keyPos = 0; keyPos + keyLength < genome.length(); keyPos++) {
        ...
        m_index.insert(key, GenomeRef(genome.name(), keyPos));
    }
}
                                               ...
                                               vector<DNAMatch> &matches) const
{
    ...
    vector<GenomeRef> candidateRefs = m_index.find(key, exactMatchOnly);
                                           ...
                                           vector<GenomeMatch> &results) const
{
    ...
    for (auto const &fragment : fragments) {
        ...
        findGenomesWithThisDNA(fragment, fragmentMatchLength, false,
                               dnaMatches);
        for (auto const &dnaMatch : dnaMatches) {
            string const genomeName = dnaMatch.genomeName;
            if (genomeMatchCountMap.find(genomeName) ==
                    genomeMatchCountMap.end()) {
...
// fragment genome into fragmentLength pieces
{
    ...
    for (int fragmentStart = 0;
         fragmentStart + fragmentLength < genome.length();
...
bool GenomeMatcher::findGenomesWithThisDNA(const string &fragment,
                                           ...
                                           vector<DNAMatch> &matches) const
{
    return m_impl->findGenomesWithThisDNA(fragment, minimumLength,
                                          exactMatchOnly, matches);
}
...
bool GenomeMatcher::findRelatedGenomes(const Genome &query,
                                       ...
                                       vector<GenomeMatch> &results) const
{
    return m_impl->findRelatedGenomes(query, fragmentMatchLength,
                                      exactMatchOnly, matchPercentThreshold,
                                      results);
}
