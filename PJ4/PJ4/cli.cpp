...
//  PJ4
//
//  Created by Jim Zenn on 3/12/19.
...
const string providedFiles[] = {
    "Ferroplasma_acidarmanus.txt", "Halobacterium_jilantaiense.txt",
    "Halorubrum_chaoviator.txt",   "Halorubrum_californiense.txt",
    "Halorientalis_regularis.txt", "Halorientalis_persicus.txt",
    "Ferroglobus_placidus.txt",    "Desulfurococcus_mucosus.txt"};
...
void findGenome(GenomeMatcher *library, bool exactMatch) {
  ...
  if (minMatchLength > sequence.size()) {
    cout << "Minimum match length must be at least the sequence length."
         << endl;
    return;
  }
  vector<DNAMatch> matches;
  if (!library->findGenomesWithThisDNA(sequence, minMatchLength, exactMatch,
                                       matches)) {
    cout << "No ";
    if (exactMatch)
      cout << " matches";
  ...
  for (const auto &m : matches)
    cout << "  length " << m.length << " position " << m.position << " in "
         << m.genomeName << endl;
}
...
void findRelatedGenomesManual(GenomeMatcher *library) {
  ...
  library->findRelatedGenomes(Genome("x", sequence), 2 * minLength,
                              exactMatchOnly, pctThreshold, matches);
  if (matches.empty()) {
...
void findRelatedGenomesFromFile(GenomeMatcher *library) {
  string filename;
  cout << "Enter name of file containing one or more genomes to find matches "
          "for: ";
  getline(cin, filename);
  if (filename.empty()) {
    cout << "No file name entered." << endl;
  ...
  for (const auto &g : genomes) {
    vector<GenomeMatch> matches;
    library->findRelatedGenomes(g, 2 * minLength, exactMatchOnly, pctThreshold,
                                matches);
    ...
    cout.precision(2);
    for (const auto &m : matches)
      cout << "     " << setw(6) << m.percentMatch << "%  " << m.genomeName
           << endl;
  }
}
...
void showMenu() {
  cout << "        Commands:" << endl;
  cout << "         c - create new genome library      s - find matching SNiPs"
       << endl;
  cout << "         a - add one genome manually        r - find related "
          "genomes (manual)"
       << endl;
  cout << "         l - load one data file             f - find related "
          "genomes (file)"
       << endl;
  cout << "         d - load all provided data files   ? - show this menu"
       << endl;
...
int main() {
  ...
  cout << "The genome library is initially empty, with a default "
          "minSearchLength of "
       << defaultMinSearchLength << endl;
  showMenu();
